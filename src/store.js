...
export default new Vuex.Store({
    ...
    actions: {
        ...
        getStaticWeathers({ commit, dispatch, state }) {
            for (let idx = 0; idx < state.cities.length; idx++) {
                dispatch('weather', { woeid: state.cities[idx].woeid, type: 'static', idxCity: idx })
            }
        },
        ...
        weather({ commit, state }, { woeid, type, idxCity }) {
            return new Promise((resolve, reject) => {
                fetch(`${process.env.API_URL.weather}${woeid}`)
                    .then(response => {
                        ...
                        response.json().then(data => {
                            ...
                            if (type === 'static') {
                                commit('setStaticWeather', { data: data.consolidated_weather[0], idxCity: idxCity })
                            } else {
    ...
    mutations: {
        ...
        setStaticWeather: (state, { weather, idxCity }) => {
